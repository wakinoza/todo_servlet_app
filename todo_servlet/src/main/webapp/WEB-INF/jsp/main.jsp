...
%>
	<p><%= pending %> : <%= todoItem.getText() %> : <a herf="Main?action=<%= todoItem.getFileName() %>">進捗を更新</a></p>
<%  } else if (todoItem.getProgress() == Progress.IN_PROGRESS) { %>
	<p><%= in_progress %> : <%= todoItem.getText() %> : <a herf="Main?action=<%= todoItem.getFileName() %>">進捗を更新</a></p>
<%  } else { %>
	<p><%= completed %> : <%= todoItem.getText() %> : <a herf="Main?action=<%= todoItem.getFileName() %>">todoを削除</a></p>
<% } %>
<% } %>
</body>
</html>